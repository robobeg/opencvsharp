...
std::vector<std::pair<int, float>> find_modes_meanshift(const std::vector<float>& hist, float sigma) {
  std::unordered_map<int, float> hash_table;
  std::vector<std::pair<int, float>> modes;

  int r = static_cast<int>(std::round(2 * sigma));
  std::vector<float> weight(2 * r + 1, 0);
  for(int i = 0; i < 2 * r + 1; ++i) {
    weight[i] = std::exp(-0.5f * (i - r) * (i - r) / sigma / sigma) / std::sqrt(2 * M_PI) / sigma;
  }

  // compute smoothed histogram
  int n = hist.size();
  std::vector<float> hist_smoothed(n, 0);
  for(int i = 0; i < n; ++i) {
