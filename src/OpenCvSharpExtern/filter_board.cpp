...
float find_minE(const Board& board, const cv::Point2i& p) {
  float minE = std::min(std::min(board.energy(p)[0], board.energy(p)[1]),
                         board.energy(p)[2]);
  if(p.x - 1 >= 0) {
...
void filter_board(const Corner& corners, std::vector<int>& used, Board& board,
                  std::vector<cv::Point2i>& proposal, float& energy, const Params& params) {
  // erase wrong corners
  while(!proposal.empty()) {
    cv::Point3i maxE_pos = board_energy(corners, board, params);
    float p_energy      = board.energy(maxE_pos.y,maxE_pos.x)[maxE_pos.z];
    if(p_energy <= energy) {
      energy = p_energy;
      break;
    }
