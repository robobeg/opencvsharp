...
//#endif
  ...
  cv::Sobel(img, img_du, -1, 1, 0, cv::FILTER_SCHARR, 1.0, 0.0, cv::BORDER_REFLECT);
  cv::Sobel(img, img_dv, -1, 0, 1, cv::FILTER_SCHARR, 1.0, 0.0, cv::BORDER_REFLECT);
  ...
  cv::cartToPolar(img_du, img_dv, img_weight, img_angle, false);
  //cv::subtract(img_angle, M_PI, img_angle, (img_angle >= M_PI));
...
/*
void gapi_image_normalization_and_gradients(const cv::GMat& imgIn, cv::GMat& imgOut
...
//#endif
    ...
    // compute image derivatives (for principal axes estimation)
    //img_du = cv::gapi::filter2D(imgIn, -1, du, cv::Point(-1, -1), 0, cv::BORDER_REFLECT);
    //img_dv = cv::gapi::filter2D(imgIn, -1, dv, cv::Point(-1, -1), 0, cv::BORDER_REFLECT);
    
    img_du = cv::gapi::Sobel(imgIn, -1, 1, 0, cv::FILTER_SCHARR, 1.0, 0.0, cv::BORDER_REFLECT);
    img_dv = cv::gapi::Sobel(imgIn, -1, 0, 1, cv::FILTER_SCHARR, 1.0, 0.0, cv::BORDER_REFLECT);

    cv::gapi::core::GMat2 polar = cv::gapi::cartToPolar(img_du, img_dv, false);
    img_weight = std::get<0>(polar);
    img_angle = std::get<1>(polar);
    ...
    //img = (img - img_min) / (img_max - img_min);
    //img.convertTo(img, -1, 1.0 / (dimg_max - dimg_min), -dimg_min / (dimg_max - dimg_min));
    imgOut = cv::gapi::normalize(imgIn, 0.0, 1.0, cv::NORM_MINMAX);
}
*/
