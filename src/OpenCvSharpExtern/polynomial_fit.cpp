...
void create_gaussin_filter_kernel(cv::Mat& kernel, float sigma) {
  int r = std::round(3 * sigma);
  kernel.create(2 * r + 1, 2 * r + 1, CV_32F);
  float sum = 0.0;
  for(int i = -r; i <= r; ++i) {
    for(int j = -r; j <= r; ++j) {
      kernel.at<float>(i + r, j + r) = std::exp(-(i * i + j * j) / 2 / sigma / sigma);
      sum += kernel.at<float>(i + r, j + r);
    }
  }
  kernel /= sum;
}
...
int create_cone_filter_kernel(cv::Mat& kernel, int r) {
  ...
  for(int i = -r; i <= r; ++i) {
    for(int j = -r; j <= r; ++j) {
      kernel.at<float>(i + r, j + r) = std::max(0.0f, r + 1 - std::sqrtf(i * i + j * j));
      sum += kernel.at<float>(i + r, j + r);
      if(kernel.at<float>(i + r, j + r) < 1e-6) {
        ++nzs;
      }
    }
  }
...
void polynomial_fit_saddle(const cv::Mat& img, int r, Corner& corners) {
  ...
  std::vector<cv::Point2f> corners_out_p, corners_out_v1, corners_out_v2;
  ...
  cv::Mat A((2 * r + 1) * (2 * r + 1) - nzs, 6, CV_32F);
  int A_row = 0;
  for(int j = -r; j <= r; ++j) {
    for(int i = -r; i <= r; ++i) {
      if(mask.at<float>(j + r, i + r) >= 1e-6) {
        A.at<float>(A_row, 0) = i * i;
        A.at<float>(A_row, 1) = j * j;
        A.at<float>(A_row, 2) = i * j;
        A.at<float>(A_row, 3) = i;
        A.at<float>(A_row, 4) = j;
        A.at<float>(A_row, 5) = 1;
        ++A_row;
      }
    }
  }
  ...
  cv::parallel_for_(cv::Range(0, corners.p.size()), [&](const cv::Range& range) -> void {
    for(int i = range.start; i < range.end; ++i) {
      ...
      float u_cur = u_init, v_cur = v_init;
      ...
      for(int num_it = 0; num_it < max_iteration; ++num_it) {
        ...
        get_image_patch_with_mask(blur_img, mask, iu, iv, u_cur-iu, v_cur-iv, r, b);
        k = invAtAAt * b;

        // check if it is still a saddle point
        float det = 4 * k.at<float>(0, 0) * k.at<float>(1, 0) - k.at<float>(2, 0) * k.at<float>(2, 0);
        if(det > 0) {
          is_saddle_point = false;
          break;
        }
        ...
        float dx = (k.at<float>(2, 0) * k.at<float>(4, 0) - 2 * k.at<float>(1, 0) * k.at<float>(3, 0)) / det;
        float dy = (k.at<float>(2, 0) * k.at<float>(3, 0) - 2 * k.at<float>(0, 0) * k.at<float>(4, 0)) / det;

        u_cur += dx;
        v_cur += dy;

        float dist = std::sqrt((u_cur - u_init) * (u_cur - u_init) + (v_cur - v_init) * (v_cur - v_init));
        if(dist > r) {
          is_saddle_point = false;
          break;
        }
...
void polynomial_fit_monkey_saddle(const cv::Mat& img, int r, Corner& corners) {
  ...
  cv::Mat A((2 * r + 1) * (2 * r + 1) - nzs, 10, CV_32F);
  int A_row = 0;
  for(int j = -r; j <= r; ++j) {
    for(int i = -r; i <= r; ++i) {
      if(mask.at<float>(j + r, i + r) >= 1e-6) {
        A.at<float>(A_row, 0) = i * i * i;
        A.at<float>(A_row, 1) = i * i * j;
        A.at<float>(A_row, 2) = i * j * j;
        A.at<float>(A_row, 3) = j * j * j;
        A.at<float>(A_row, 4) = i * i;
        A.at<float>(A_row, 5) = i * j;
        A.at<float>(A_row, 6) = j * j;
        A.at<float>(A_row, 7) = i;
        A.at<float>(A_row, 8) = j;
        A.at<float>(A_row, 9) = 1;
        ++A_row;
      }
    }
  }
  ...
  cv::parallel_for_(cv::Range(0, corners.p.size()), [&](const cv::Range& range) -> void {
    for(int i = range.start; i < range.end; ++i) {
      ...
      for(int num_it = 0; num_it < max_iteration; ++num_it) {
        ...
        float det = 3 * (k.at<float>(0, 0) * k.at<float>(2, 0) + k.at<float>(1, 0) * k.at<float>(3, 0)) -
                     (k.at<float>(1, 0) * k.at<float>(1, 0) + k.at<float>(2, 0) * k.at<float>(2, 0));
        if(det > 0) {
          is_monkey_saddle = false;
          break;
        }
        ...
        cv::Mat tmp_a = (cv::Mat_<float>(3, 2) << 3.0 * k.at<float>(0, 0), k.at<float>(1, 0),
                         2.0 * k.at<float>(1, 0), 2.0 * k.at<float>(2, 0),
                         k.at<float>(2, 0), 3.0 * k.at<float>(3, 0));
        cv::Mat tmp_b = (cv::Mat_<float>(3, 1) << -k.at<float>(4, 0), -k.at<float>(5, 0), -k.at<float>(6, 0));
        cv::Mat tmp_x = (tmp_a.t() * tmp_a).inv() * tmp_a.t() * tmp_b;
        float dx     = tmp_x.at<float>(0, 0);
        float dy     = tmp_x.at<float>(1, 0);
