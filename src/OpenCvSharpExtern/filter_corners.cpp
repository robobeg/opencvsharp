  ...
  cv::parallel_for_(cv::Range(0, corners.p.size()), [&](const cv::Range& range) -> void {
    for(int i = range.start; i < range.end; ++i) {
      ...
      for(int j = 0; j < n_cicle; ++j) {
        ...
        c[j]         = img.at<float>(circle_v, circle_u);
      }
      auto minmax  = std::minmax_element(c.begin(), c.end());
      float min_c = *minmax.first, max_c = *minmax.second;
      for(int j = 0; j < n_cicle; ++j) {
      ...
      img_weight_sub.forEach<float>([&tmp_maxval](float& val, const int* pos) -> void {
        val = val < 0.5 * tmp_maxval ? 0 : val;
      });

      // create histogram
      std::vector<float> angle_hist(n_bin, 0);
      for(int j2 = top_left_v; j2 <= bottom_right_v; ++j2) {
        for(int i2 = top_left_u; i2 <= bottom_right_u; ++i2) {
          int bin = static_cast<int>(std::floor(img_angle.at<float>(j2, i2) / (M_PI / n_bin))) % n_bin;
          angle_hist[bin] += img_weight_sub.at<float>(j2 - center_v + r, i2 - center_u + r);
        }
      }
