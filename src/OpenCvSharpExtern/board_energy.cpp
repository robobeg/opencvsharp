...
cv::Point3i board_energy(const Corner& corners, Board& board, const Params& params) {
  ...
  float E_corners = -1.0f * board.num();

  // energy: structure
  float max_E_structure = std::numeric_limits<float>::min();
  ...
  for(int i = 0; i < board.rows(); ++i) {
    for(int j = 0; j < board.cols() - 2; ++j) {
      ...
      if(idx1 >= 0 && idx2 >= 0 && idx3 >= 0) {
        const cv::Point2f& x1 = corners.p[idx1];
        const cv::Point2f& x2 = corners.p[idx2];
        const cv::Point2f& x3 = corners.p[idx3];
        float E_structure    = mynorm(x1 + x3 - 2 * x2) / mynorm(x1 - x3);
